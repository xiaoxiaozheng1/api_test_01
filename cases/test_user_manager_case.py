...
class TestUserManger(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # cls.user.login()  因为登录不是每次都登录，故放在入口脚本run_case这里
        cls.username = 'testaa01'
        cls.new_username = 'testa11'
    ...
    def test01_add_user(self):
        ...
        if data:
            ...
            TestUserManger.user_id = self.user_id
            logger.info("获取添加管理员的用户id:{}".format(self.username))
    ...
    def test02_edit_username(self):
        # 1.定义编辑的次测试数据
        # new_username = 'testaa01'
        # 2.请求编辑管理员的接口
        actual_result = self.user.edit_user(TestUserManger.user_id, self.new_username, password=123456)
        # 3.进行断言
        self.assertEqual(0, actual_result['errno'])
    ...
    def test04_delete_user(self):
        ...
        actual_result = self.user.delete_user(TestUserManger.user_id, self.new_username)
        # 3.断言返回结果
        self.assertEqual(0, actual_result['errno'])
        # self.assertEqual()
