...
class Base():
    ...
    def post(self, url, data):
        ...
        """
        result = None
        # requests.post(url, json=data, headers=None)
    ...
    def get_headers(self):
        """
        处理请求头：
        :return:返回的时字典格式的响应头，多是包含了Content-Type，X-Litemall-Admin-Token
        """
        headers = {'Content-Type': 'application/json'}  # 默认headers初始值
    ...
    def login(self):
        """
        通过调用登录接口获取token值，将其进行缓存，其他接口使用是=时直接从缓存中欧取数。
        若没有渠道，再调用登录，再将token值放在缓存中。
        :return:
        """
